...
<template>
	...
	>
		<div class="sticky top-0 z-10 flex items-center gap-2 bg-surface-white py-1.5">
			<FormControl
				type="checkbox"
		...
		>
			...
			>
				<template #prefix>
					<SearchIcon class="h-4 w-4 text-ink-gray-6" stroke-width="1.5" />
				</template>
				<template #suffix>
					<LoadingIndicator v-if="tableStore.loading" class="h-4 w-4 text-ink-gray-6" />
				</template>
			</FormControl>
			<div
					...
					<div class="group flex items-center gap-2">
						...
						>
							...
							<p
								...
								"
								class="invisible cursor-pointer text-xs leading-5 text-ink-gray-6 transition-all hover:underline group-hover:visible motion-reduce:transition-none"
								:class="
			...
			<div v-if="!expandedDataSourceTables.length" class="text-xs text-ink-gray-6">
				No tables found
			</div>
		</div>
	</div>
