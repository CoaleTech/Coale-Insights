		...
		<template #target="{ togglePopover, isOpen }">
			<div class="group relative flex cursor-pointer items-center gap-2">
				<Button
					variant="outline"
					label="Add Operation"
					class="-ml-[14px] !h-6 !gap-1.5 bg-surface-white !px-2 text-p-xs"
					@click="togglePopover"
				>
					<template #prefix>
					<Plus class="h-4 w-4 text-ink-gray-7" stroke-width="1.5" />
					</template>
				</Button>
			</div>
		</template>
					...
					>
						<template #prefix>
							<div class="flex items-start gap-2 py-2">
								<component
									:is="button.icon"
								class="h-4.5 w-4.5 flex-shrink-0 text-ink-gray-7"
									stroke-width="1.5"
								/>
