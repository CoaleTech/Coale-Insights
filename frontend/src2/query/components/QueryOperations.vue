...
const Element = (_: any, { slots }: any) => {
	return (
		<div class="w-fit truncate rounded border border-outline-orange-1 bg-surface-orange-1 px-1 py-0.5 font-mono text-xs text-highlight opacity-90">
			{slots.default?.()}
		</div>
	)
}
...
const SourceInfo = (props: any) => {
	...
	return (
		<div class="flex items-baseline gap-1 text-ink-gray-7">
			<p>Select</p>
...
const JoinInfo = (props: any) => {
	...
	return (
		<div class="flex flex-wrap items-baseline gap-1 text-ink-gray-7">
...
<template>
	<div v-if="query.doc.operations.length" class="flex w-full flex-col px-3.5 py-3">
		...
		<div class="relative ml-3 flex flex-col-reverse gap-3 border-l border-outline-gray-2 text-sm">
			<template v-for="(op, idx) in operations" :key="idx">
				...
				>
					...
					>
						...
						<div
							class="absolute right-0 flex h-full flex-shrink-0 items-center bg-surface-white opacity-0 transition-all group-hover:opacity-100 motion-reduce:transition-none"
						>
							<Button
								variant="ghost"
