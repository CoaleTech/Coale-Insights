...
import { Plus, SearchIcon, XIcon } from 'lucide-vue-next'
	...
	columns: [
		...
		},
		{
			...
			prefix: (props: any) => {
				...
				if (user.invitation_status) {
					color =
						user.invitation_status === 'Pending' ? 'text-warn' : 'text-neg'
				} else {
					color = props.row.enabled ? 'text-pos' : 'text-ink-gray-4'
				}
			...
			<div class="flex flex-col gap-4">
				<div class="flex flex-wrap gap-1 rounded bg-surface-gray-2 p-0.5">
					...
					<div class="min-w-[10rem] flex-1">
						...
						class="h-7 w-full rounded border-none bg-surface-gray-2 py-1.5 pl-2 pr-2 text-base text-ink-gray-8 placeholder-ink-gray-4 transition-colors focus:outline-none focus:ring-0 focus-visible:outline-none focus-visible:ring-0 motion-reduce:transition-none"
						/>
					</div>
				</div>
			</div>
