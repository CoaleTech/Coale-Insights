	...
	<div class="flex flex-col px-3.5 pt-3">
		<div class="mb-1 flex h-6 items-center justify-between">
			...
			<div v-if="!editingFolderName" class="flex gap-1">
				...
				>
			<Folder class="h-4 w-4 text-ink-gray-6" stroke-width="1.5" />
				</Button>
				<Button 
					class="!h-fit !p-1" 
					variant="ghost" 
		...
		>
		<div class="text-xs text-ink-gray-6">{{ section.emptyMessage }}</div>
		</div>
		...
		<div v-else class="flex flex-col border-b pb-3">
			...
			>
				...
				class="absolute -top-0.5 left-0 right-0 h-0.5 bg-accent transition-all motion-reduce:transition-none"
				/>

				<div
					class="group w-full cursor-pointer rounded transition-all hover:bg-surface-gray-2 drag-item motion-reduce:transition-none"
					:class="[
				...
				>
					...
					>
						...
						<button
						class="invisible cursor-pointer rounded px-1.5 py-1 transition-all hover:bg-surface-gray-2 group-hover:visible motion-reduce:transition-none"
				...
				class="absolute -bottom-0.5 left-0 right-0 h-0.5 bg-accent transition-all motion-reduce:transition-none"
				/>
			</div>
			...
			<div
				...
				:class="{
				'bg-surface-blue-1 ring-1 ring-outline-blue-1': dragOverFolder === folder.name
				}"
			>
				<div
				:class="['group flex h-7.5 cursor-pointer items-center justify-between rounded px-1.5 mb-0.5 transition-all hover:bg-surface-gray-2', 
				editingFolderName === folder.name ? 'ring-1 ring-outline-gray-3' : '', 'motion-reduce:transition-none']"
