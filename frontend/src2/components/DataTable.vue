...
const colorByPercentage = {
	0: 'bg-surface-white text-ink-gray-9',
...
function getColorClass(colorName: string): string {
	if (!colorName) return 'bg-surface-gray-4'
	...
	switch (colorName.toLowerCase()) {
		...
		default:
			return colorName.startsWith('bg-') ? colorName : 'bg-surface-gray-4'
	}
}
...
<template>
		...
		<div class="w-full flex-1 overflow-y-auto">
			...
			<thead ref="$header" class="sticky top-0 z-10 bg-surface-gray-1">
					<tr v-for="headerRow in headers">
						<td
						class="sticky left-0 z-10 h-8 whitespace-nowrap border-b border-r bg-surface-gray-1 px-3"
						...
						<td
							...
							:class="[
								header.isLast && isNumberColumn(header.column.name)
									? 'text-right'
									: 'text-left',
							isStickyColumn(header.column.name) ? 'sticky z-10 bg-surface-gray-1' : '',
							]"
						...
						<td v-if="props.enableNewColumn" class="h-8 border-b border-r">
							...
							>
								<template #icon>
							<Plus class="size-4 text-ink-gray-7" :stroke-width="1.5" />
								</template>
							</Button>
							<slot
								v-if="showNewColumn"
					...
					<tr v-if="props.showFilterRow">
						...
						:class="isStickyColumn(column.name) ? 'sticky z-10 bg-surface-gray-1' : ''"
						...
						class="[&_input]:h-6 [&_input]:bg-surface-gray-3/80"
							>
								<template #prefix>
							<Search class="h-4 w-4 text-ink-gray-4" stroke-width="1.5" />
								</template>
							</FormControl>
						</td>
						<td
				...
				<tbody>
					...
					>
						<td
						class="tnum z-1 sticky left-0 h-8 whitespace-nowrap border-b border-r bg-surface-white px-3 text-right text-xs"
							width="1px"
							height="30px"
						>
						...
						class="h-8 max-w-[24rem] truncate border-b border-r px-3 text-ink-gray-8"
							:class="[
						...
						isStickyColumn(col.name) ? 'sticky z-1 bg-surface-white' : '',
							]"
					...
					class="sticky bottom-0 z-10 border-b bg-surface-white"
					>
						<td class="h-8 whitespace-nowrap border-r border-t px-3"></td>
						<td
							v-for="col in props.columns"
						class="h-8 truncate border-r border-t px-3 font-bold text-ink-gray-8"
							:class="[
								isNumberColumn(col.name) ? 'tnum text-right' : 'text-left',
						isStickyColumn(col.name) ? 'sticky z-10 bg-surface-white' : '',
							]"
							:style="getStickyColumnStyle(col.name)"
						>
							{{
					...
					<div class="flex items-center gap-2">
						...
						<p class="tnum text-sm text-ink-gray-6">
							...
							<div class="flex gap-2">
								...
								>
								<ChevronLeft class="h-4 w-4 text-ink-gray-7" stroke-width="1.5" />
								</Button>
								<Button
									variant="ghost"
									@click="page.next"
	...
	<div v-else class="flex h-full w-full items-center justify-center">
		<div class="flex flex-col items-center gap-2">
			<Table2Icon class="h-16 w-16 text-ink-gray-1" stroke-width="1.5" />
    ...
    <div
        v-if="props.loading"
        class="absolute top-10 z-10 flex h-[calc(100%-2rem)] w-full items-center justify-center rounded bg-white-overlay-300 backdrop-blur-sm"
    >
        <LoadingIndicator class="h-8 w-8 text-ink-gray-7" />
    </div>
</template>
