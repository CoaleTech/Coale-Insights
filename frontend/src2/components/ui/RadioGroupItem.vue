<template>
  <div class="relative flex items-start">
    ...
    >
      <span 
      class="flex h-4 w-4 aspect-square items-center justify-center rounded-full border transition-colors duration-200 ease-in-out motion-reduce:transition-none"
        :class="[
          isChecked 
      ...
      >
        <span 
          class="h-2 w-2 rounded-full transition-opacity duration-200 motion-reduce:transition-none"
          :class="[
            isChecked ? 'opacity-100' : 'opacity-0',
            isDisabled && isChecked ? 'bg-surface-gray-4' : 'bg-surface-white'
          ]"
        />
      </span>
      <slot />
